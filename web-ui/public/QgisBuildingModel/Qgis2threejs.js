...
Q3D.Config = {
  ...
  viewpoint: {
    // z-up
    pos: new THREE.Vector3(0, -25, 25), // initial camera position
...
Q3D.Scene.prototype.adjustZShift = function () {
  ...
  var zmin =
    box.min.y === Infinity
      ? 0
      : box.min.y / this.userData.zScale - this.userData.zShift;
  ...
  app.init = function (container) {
    ...
    if ("popup" in app.urlParams) {
      ...
      window.open(
        ...
        "popup",
        "width=" + app.urlParams.width + ",height=" + app.urlParams.height
      );
    ...
    if (typeof THREE.OrbitControls !== "undefined") {
      ...
      controls.moveForward = function (delta) {
        ...
        var targetDistance =
          offset.length() *
          Math.tan(((controls.object.fov / 2) * Math.PI) / 180.0);
        offset.z = 0;
        offset.normalize();
        offset.multiplyScalar(
          (-2 * delta * targetDistance) / app.renderer.domElement.clientHeight
        );
...
Q3D.DEMBlock.prototype = {
  ...
  buildFrame: function (layer, parent, material, z0) {
    ...
    var v = [-hw, -hh, e0, hw, -hh, e0, hw, hh, e0, -hw, hh, e0, -hw, -hh, e0];
    ...
    if (jsonObject.sides || jsonObject.frame) {
      ...
      var buildSides = function () {
        ...
        if (jsonObject.sides) {
          ...
          _this.blocks[index].buildSides(
            _this,
            m,
            material.mtl,
            Q3D.Config.dem.side.bottomZ
          );
        }
        // build frame
        if (jsonObject.frame) {
          ...
          _this.blocks[index].buildFrame(
            _this,
            m,
            material.mtl,
            Q3D.Config.dem.frame.bottomZ
          );
        }
        _this.requestRender();
      };
...
Q3D.DEMLayer.prototype.setSideVisible = function (visible) {
  ...
  this.objectGroup.traverse(function (obj) {
    if (obj.name == "side" || obj.name == "bottom" || obj.name == "frame")
      obj.visible = visible;
  });
};
